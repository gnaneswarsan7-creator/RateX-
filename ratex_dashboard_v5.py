...
class RateXEngine:
    ...
    def multi_head_attention(self, row, review_cols):
        ...
        var_penalty = max(1.0 - np.std(raw) * 0.3, 0.7)
        return {
    ...
    def score_org(self, row):
        ...
        weighted = float(np.mean(raw)) * 0.75 + fo["score"] * 0.25
...
if "Employee" in mode or "Combined" in mode:
    if emp_raw is not None:
        ...
        dim_scores = {c.replace("_Review","").replace("_"," "):
                      emp[c].dropna().apply(lambda x: engine_tmp.token_attention_score(str(x))["score"]).mean()
                      for c in rcols}
