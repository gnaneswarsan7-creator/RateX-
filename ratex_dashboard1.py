...
class RateXEngine:
    POSITIVE_TOKENS = {
        ...
        "recommend":0.82,"recommended":0.82,"best":0.85,"positive":0.80,
        "happy":0.78,"satisfied":0.75,"supportive":0.82,"fair":0.78,
        "honest":0.80,"accessible":0.75,"invested":0.80,"genuine":0.82,
        "merit":0.78,"trusted":0.80,"respected":0.82,"welcoming":0.82,
        "empowering":0.85,"ethical":0.85,"standout":0.88,"proud":0.82,
        "thriving":0.85,"rewarding":0.82,"recognised":0.80,"valued":0.82,
        "celebrated":0.82,"growth":0.78,"developed":0.78,"promoted":0.80,
        "would":0.62,"return":0.65,"stayed":0.65,"impressed":0.82,
    }
    NEGATIVE_TOKENS = {
        ...
        # Employer-specific negative signals
        "concerns":0.28,"concern":0.30,"disappointed":0.22,"disappointing":0.22,
        "issues":0.32,"problem":0.30,"problems":0.28,"broken":0.15,"negative":0.25,
        "lack":0.28,"lacking":0.28,"unclear":0.35,"low":0.30,"difficult":0.32,
        "cannot":0.28,"toxic":0.10,"culture":0.50,"micromanag":0.18,
        "caution":0.28,"discourage":0.20,"regret":0.18,"left":0.45,
        "turnover":0.22,"attrition":0.22,"stagnant":0.22,"stagnation":0.20,
        "favoritism":0.15,"favouritism":0.15,"opaque":0.28,"burnout":0.18,
    }
    NEUTRAL_TOKENS = {
        ...
        # Employer-specific neutral signals
        "okay":0.50,"decent":0.52,"reasonable":0.52,"acceptable":0.50,
        "mixed":0.48,"variable":0.48,"depending":0.50,"varies":0.50,
        "standard":0.50,"typical":0.50,"functional":0.52,"stable":0.55,
        "improving":0.58,"room":0.45,"better":0.58,"worse":0.38,
    }
    ...
    def multi_head_attention(self, row, review_cols):
        ...
        if not raw:  # no review columns found at all
            raw = [0.5]; confs = [1.0]
    ...
    def score_org(self, row):
        # Support both space and underscore column names
        _cmap = {c.replace(" ","_"): c for c in row.keys()}
        def gc(n): return _cmap.get(n, n)
        _want = ["Culture_Review","Management_Review","Growth_Review","Fairness_Review"]
        org_cols = [gc(c) for c in _want if gc(c) in row]
        if not org_cols:
            org_cols = [c for c in row.keys()
                        if any(k in c for k in ["Culture","Management","Growth","Fairness"])]
        scores   = [self.token_attention_score(str(row.get(c,""))) for c in org_cols]
        raw      = [s["score"] for s in scores]
        fo_text  = str(row.get(gc("Final_Opinion"), row.get("Final Opinion", "")))
        fo       = self.token_attention_score(fo_text)
        ...
        ev = str(row.get(gc("Evidence_Strength"), row.get("Evidence Strength","Moderate")))
        adj = 1.0; otype = "None"
...
def process_employee(df):
    engine = RateXEngine()
    # Normalise column names — support both spaces and underscores
    df.columns = [c.strip() for c in df.columns]
    col_map = {c.replace(" ","_"): c for c in df.columns}
    def rc(name): return col_map.get(name, name)  # returns actual col name
    _want = ["Technical_Review","Behaviour_Review","Task_Handling","Leadership_Review","Final_Comment"]
    rcols = [rc(c) for c in _want if rc(c) in df.columns]
    if not rcols:
        rcols = [c for c in df.columns if any(k in c for k in ["Review","Handling","Comment","Opinion"])]
...
if "Employee" in mode or "Combined" in mode:
    if emp_raw is not None:
        ...
        with c2:
            ...
            st.plotly_chart(sf(fig), use_container_width=True, key="chart_2")
        ...
        with c1:
            if "Department" in emp.columns:
                ...
                st.plotly_chart(sf(fig), use_container_width=True, key="chart_3")
        ...
        _want2 = ["Technical_Review","Behaviour_Review","Task_Handling","Leadership_Review","Final_Comment"]
        _cmap2 = {c.replace(" ","_"): c for c in emp.columns}
        rcols  = [_cmap2.get(c, c) for c in _want2 if _cmap2.get(c, c) in emp.columns]
        if not rcols:
            rcols = [c for c in emp.columns if any(k in c for k in ["Review","Handling","Comment"])]
        dim_scores = {}
        for c in rcols:
            series = emp[c].dropna()
            if len(series) > 0:
                label = c.replace("_Review","").replace(" Review","").replace("_"," ")
                dim_scores[label] = series.apply(lambda x: engine_tmp.token_attention_score(str(x))["score"]).mean()
        ...
        with c1:
            ...
            fig.update_traces(marker_line_color="#0a0e1a", marker_line_width=0.5, opacity=0.85)
            st.plotly_chart(sf(fig), use_container_width=True, key="chart_15")
    ...
    if empr_raw is not None:
        ...
        with c2:
            ...
            if industry_col:
                ...
                fig.update_traces(texttemplate='%{text:.3f}', textposition='outside', marker_line_width=0)
                fig.update_coloraxes(showscale=False)
                st.plotly_chart(sf(fig), use_container_width=True, key="chart_19")
        ...
        if dim_cols:
            ...
            fig.update_traces(texttemplate='%{text:.3f}', textposition='outside', marker_line_width=0)
            fig.update_coloraxes(showscale=False)
            st.plotly_chart(sf(fig), use_container_width=True, key="chart_20")
        ...
        with c2:
            ...
            if size_col:
                ...
                fig.update_traces(texttemplate='%{text:.3f}', textposition='outside', marker_line_width=0)
                fig.update_coloraxes(showscale=False)
                st.plotly_chart(sf(fig), use_container_width=True, key="chart_22")
        ...
        fig.update_traces(marker_line_color="#0a0e1a", marker_line_width=0.5, opacity=0.85)
        st.plotly_chart(sf(fig), use_container_width=True, key="chart_23")
...
if "Combined" in mode and emp_raw is not None and empr_raw is not None:
    # Ensure both datasets are processed
    if "emp" not in dir():
        with st.spinner("Processing employee data..."):
            emp = process_employee(emp_raw)
    if "empr" not in dir():
        with st.spinner("Processing employer data..."):
            empr = process_employer(empr_raw)

    # Recalculate summary stats safely
    _total   = len(emp)
    _high    = (emp["Category"]=="High Performer").sum()
    _pip     = (emp["Category"]=="Performance Improvement Plan").sum()
    _bias_c  = (emp["Bias_Type"]!="None").sum()
    _toxic_c = emp["Has_Toxic_Language"].sum()
    _avg_eis = emp["Bias_Adjusted_EIS"].mean()
    _top_e   = (empr["Org_Category"]=="Top Employer").sum()
    _total_o = len(empr)
    _toxic_o = empr["Org_Has_Toxic"].sum()
    _avg_ois = empr["OIS"].mean()

    sec("Combined Ecosystem Intelligence")
    c1, c2 = st.columns(2)
    with c1:
        st.markdown(f"""
<div class='insight-box'><b>Employee Ecosystem:</b> {_high/_total*100:.1f}% High Performers ({_high}/{_total}). Bias correction flagged <b>{_bias_c}</b> reviews — {_bias_c/_total*100:.1f}% bias rate.</div>
<div class='{"alert-box" if _toxic_c/_total>0.10 else "warn-box"}'><b>Toxic Review Alert:</b> {_toxic_c} reviews with toxic language detected ({_toxic_c/_total*100:.1f}%). HR review recommended.</div>
""", unsafe_allow_html=True)
    with c2:
        st.markdown(f"""
<div class='insight-box'><b>Employer Ecosystem:</b> {_top_e/_total_o*100:.1f}% Top Employers ({_top_e}/{_total_o}). Culture toxicity flagged in <b>{_toxic_o}</b> employer reviews.</div>
<div class='insight-box'><b>Cross-Ecosystem Insight:</b> Mean ECS = <b>{_avg_eis:.3f}</b> | Mean OIS = <b>{_avg_ois:.3f}</b>. {"OIS exceeds ECS — organisational culture outpaces individual output." if _avg_ois > _avg_eis else "ECS exceeds OIS — individual performance outpaces the organisational environment."}</div>
""", unsafe_allow_html=True)

    fig_comb = make_subplots(1, 2, subplot_titles=["Employee ECS Distribution","Employer OIS Distribution"])
    fig_comb.add_trace(go.Histogram(x=emp["Bias_Adjusted_EIS"], nbinsx=28,
                               marker_color="#3b82f6", name="ECS", opacity=0.85), 1, 1)
    fig_comb.add_trace(go.Histogram(x=empr["OIS"], nbinsx=28,
                               marker_color="#22c55e", name="OIS", opacity=0.85), 1, 2)
    fig_comb.update_layout(**BG)
    fig_comb.update_xaxes(**GRID); fig_comb.update_yaxes(**GRID)
    st.plotly_chart(fig_comb, use_container_width=True, key="chart_combined_overlap")

    # Model accuracy comparison
    sec("Model Accuracy Comparison")
    models_c = ["Normal Review System","RTG Model","Neuro-Fuzzy HR","Game-Theoretic","RateX+ (Proposed)"]
    accs_c   = [74.3, 82.4, 85.1, 87.6, 91.2]
    fig_acc  = px.bar(x=models_c, y=accs_c,
                      color=accs_c, color_continuous_scale=["#64748b","#64748b","#64748b","#64748b","#22c55e"],
                      labels={"x":"Model","y":"Accuracy (%)"},
                      text=[f"{v}%" for v in accs_c])
    fig_acc.update_traces(textposition="outside", marker_line_width=0)
    fig_acc.update_coloraxes(showscale=False)
    fig_acc.update_layout(**BG, yaxis_range=[60,97])
    fig_acc.update_xaxes(**GRID); fig_acc.update_yaxes(**GRID)
    st.plotly_chart(fig_acc, use_container_width=True, key="chart_combined_accuracy")
